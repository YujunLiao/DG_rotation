...
class JigsawDataset(data.Dataset):
    def __init__(self, names, labels, jig_classes=100, img_transformer=None, tile_transformer=None, patches=True, bias_whole_image=None):
        ...
        # TODO(lyj):What is inside the permutations
        # Code for the make grid of the picture
        # Get the sequence of random index
        self.permutations = self.__retrieve_permutations(jig_classes)
        # Parameter to make grid
        self.grid_size = 3
        # The chance to appear the ordered picture
        self.bias_whole_image = bias_whole_image
        # Patch based or not
        if patches:
            self.patch_size = 64
        # Parameter for the augmentation
