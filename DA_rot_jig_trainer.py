...
from trainer_utils.data_loader.DARotJigDataLoader import DARotationDataLoader
...
class DARotationTrainer:
    ...
    def _do_epoch(self):
        ...
        for i, (data_from_source_domain, data_from_target_domain) \
            ...
            (data, rotation_label, jigsaw_s_data, jigsaw_s_label, class_label), domain_index_of_images_in_this_patch = data_from_source_domain
            ...
            jigsaw_s_data, jigsaw_s_label = jigsaw_s_data.to(self.device), jigsaw_s_label.to(self.device)

            (target_domain_data, target_domain_rotation_label, jigsaw_t_data, jigsaw_t_label,_), _ = data_from_target_domain
            ...
            jigsaw_t_data, jigsaw_t_label = jigsaw_t_data.to(self.device), jigsaw_t_label.to(self.device)


            self.optimizer.zero_grad()

            rotation_predict_label, _, class_predict_label = self.model(data)  # , lambda_val=lambda_val)
            _, jigsaw_predict_label, _ = self.model(jigsaw_s_data)
            unsupervised_task_loss = criterion(rotation_predict_label, rotation_label)
            unsupervised_task_loss_j = criterion(jigsaw_predict_label, jigsaw_s_label)


            target_domain_rotation_predict_label, _, target_domain_class_predict_label = self.model(target_domain_data)  # , lambda_val=lambda_val)
            _, jigsaw_predict_label_t, _ = self.model(jigsaw_t_data)
            ...
            t_unsupervised_task_loss_j = criterion(jigsaw_predict_label_t, jigsaw_t_label)
            ...
            + unsupervised_task_loss_j * self.training_arguments.unsupervised_task_weight_j \
            + t_unsupervised_task_loss_j * self.training_arguments.target_domain_unsupervised_task_loss_weight_j \
            ...
            del unsupervised_task_loss_j, t_unsupervised_task_loss_j
        self.model.eval()
        with torch.no_grad():
    ...
    def do_training(self):
        ...
        print("target_jigsaw_weight:", self.training_arguments.target_domain_unsupervised_task_loss_weight_j)
        ...
        self.output_manager.write_to_output_file([
            ...
            "target_jigsaw_weight:" + str(self.training_arguments.target_domain_unsupervised_task_loss_weight_j),
...
if __name__ == "__main__":
    ...
    for parameter_pair in DA_rotation_training_argument.training_arguments.parameters_lists:
        ...
        DA_rotation_training_argument.training_arguments.unsupervised_task_weight_j=parameter_pair[4]
        DA_rotation_training_argument.training_arguments.target_domain_unsupervised_task_loss_weight_j=parameter_pair[5]
        ...
        output_file_path = \
            ...
            str(DA_rotation_training_argument.training_arguments.unsupervised_task_weight_j) + '_' + \
            str(DA_rotation_training_argument.training_arguments.target_domain_unsupervised_task_loss_weight_j) + '/'
