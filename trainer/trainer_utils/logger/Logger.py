...
class Logger():
    ...
    def log(self, it, iters, losses, samples_right, total_samples):
        ...
        if it % self.update_f == 0:
            print("%d/%d of epoch %d/%d %s - acc %s [bs:%d]" % (it, iters, self.current_epoch, self.epochs_number, loss_string,
    ...
    def save_best(self, val_test, best_test):
        print("Accuracy on test set when the accuracy on validation set is highest:%.3f" %val_test)
        print("Highest accuracy on test set:%.3f" %best_test)
